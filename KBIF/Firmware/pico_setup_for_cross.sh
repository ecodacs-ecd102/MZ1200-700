...
export TMP=/tmp/tmp-pi
rm -rf "$TMP"
mkdir -p "$TMP"

# binutils target
. /usr/src/CROSS/path32p.sh
export PICO_TARGET_ARCH=$TARGET_ARCH
...
do
    ...
    if [ -d $DEST ]; then
        ...
        # Define PICO_SDK_PATH in $CWD/env.sh
        VARNAME="PICO_${REPO^^}_PATH"
        echo "Adding $VARNAME to $CWD/env.sh"
        sed -i -r -e "/^\s*export\s+$VARNAME=/d" "$CWD/env.sh"
        echo "export $VARNAME=$DEST" >> "$CWD/env.sh"
        export ${VARNAME}=$DEST
    fi
done
...
#
# kaokun: Update TinyUSB
#
cd $CWD/pico/pico-sdk/lib/tinyusb/hw/mcu/raspberry_pi
mv Pico-PIO-USB Pico-PIO-USB.orig
git clone https://github.com/sekigon-gonnoc/Pico-PIO-USB.git
cd $CWD
cat $CWD/*.patch | patch -p0 -b --verbose
